...
import { parse_url2, IUrlObject } from 'root/lib/func/parse_url';
import { IWindow } from 'root/lib/core/demo';
...
export let log_count = 1;

export let wordMapHttp = {
...
export type ISelectFile = {
	filename,
	path,
	fs_id,
	isdir,
};
export type ISelectFileList = ISelectFile[];

export function init($, _url_obj?: IUrlObject)
{
	makeJQueryPlugin($);

	switch(detectPage()){
		case 'disk':
			let panHelper = new PanHelper(_url_obj);
			panHelper.init();
			return;
		case 'share':
		case 's':
			let panShareHelper = new PanShareHelper(_url_obj);
			panShareHelper.init();
			return;
		default:
			return;
...
export function makeJQueryPlugin($)
{
...
export function PanHelper(_url_obj?: IUrlObject)
{
	var yunData, sign, timestamp, bdstoken, logid, fid_list;
	var fileList = [], selectFileList: ISelectFileList = [], batchLinkList = [], batchLinkListAll = [], linkList = [],
	...
	function getListGridStatus()
	{
		...
		return $('div.' + wordMap['list-grid-switch']).hasClass(wordMap['list-switched-on'])
			? 'list'
			: ($('div.' + wordMap['list-grid-switch']).hasClass(wordMap['grid-switched-on']) ? 'grid' : 'list');
	}
	...
	function addButton()
	{
		...
		$directbutton_menu.append($directbutton_download_button)
			.append($directbutton_link_button)
			.append($directbutton_batchhttplink_button)
			.append($directbutton_batchhttpslink_button);
		...
		$apibutton_menu.append($apibutton_download_button)
			.append($apibutton_link_button)
			.append($apibutton_batchhttplink_button)
			.append($apibutton_batchhttpslink_button);
		...
		$outerlinkbutton_menu.append($outerlinkbutton_download_button)
			.append($outerlinkbutton_link_button)
			.append($outerlinkbutton_batchlink_button);
...
// 2017-07-16 Tenmetsu 针对下载助手无法显示
	...
	function batchClick(event)
	{
		...
		var id = event.target.id;
		var linkType, tip;
		linkType = id.indexOf('https') == -1
			? (id.indexOf('http') == -1 ? location.protocol + ':' : 'http:')
			: 'https:';
		batchLinkList = [];
		batchLinkListAll = [];
		if (id.indexOf('direct') != -1)
...
export function PanShareHelper(_url_obj?: IUrlObject)
{
	var yunData,
		sign,
		timestamp,
	...
	var fileList = [], selectFileList: ISelectFileList = [];
	var panAPIUrl = location.protocol + "//" + location.host + "/api/";
...
export function getLogID()
{
	...
	function m(e)
	{
		...
		let t = e.charCodeAt(0) << 16 | (e.length > 1 ? e.charCodeAt(1) : 0) << 8 | (e.length > 2
			? e.charCodeAt(2)
			: 0);
		let o = [
			u.charAt(t >>> 18),
...
export function base64Encode(t)
{
...
export function Dialog(options?)
{
	...
	function createDialog()
	{
		...
		$dialog_copy_button.click(function ()
		{
			...
			GMApi.GM_setClipboard(content, 'text');
			alert('已将链接复制到剪贴板！');
		});
...
export function VCodeDialog(refreshVCode, confirmClick)
{
	var dialog, shadow;
